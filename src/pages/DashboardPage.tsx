    ...
    <DashboardLayout>
      <div className="max-w-6xl mx-auto px-2 sm:px-4 py-4 sm:py-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mt-4 sm:mt-8">
          {/* Create New Resume Card - Make it more tappable on mobile */}
          <div 
            onClick={handleCreateNew}
            className="border border-dashed border-gray-600 rounded-lg p-4 sm:p-6 cursor-pointer hover:border-gray-400 transition-all duration-300 flex flex-col items-center justify-center min-h-[150px] sm:min-h-[200px] bg-gray-800/20"
          >
            <h3 className="text-lg sm:text-xl font-medium text-gray-300 mb-2">Create new resume</h3>
            <PlusCircle size={32} className="text-gray-500 mt-4" />
          </div>
          
          {/* Resume Cards - Improve mobile UI */}
          {resumes.length > 0 ? (
            resumes.map((resume) => (
              <div key={resume.id} className="bg-gray-800/40 rounded-lg overflow-hidden">
                <div className="relative">
                  ...
                  >
                    <div className="mb-2 flex justify-between items-start">
                      <div className="max-w-[70%]">
                        <h3 className="text-base sm:text-lg font-normal text-gray-300 break-words">{resume.title.toLowerCase()}</h3>
                        <p className="text-xs sm:text-sm text-gray-500">Edited {getTimeAgo(resume.updated_at)}</p>
                      </div>
                      <div className="flex gap-2 sm:gap-3 items-center">
                        {/* Mobile-friendly action buttons with increased tap targets */}
                        <button 
                        ...
                        <button 
                          ...
                          className="text-gray-500 hover:text-gray-300 p-2 rounded-full transition-colors"
                        >
                    ...
                    <div className="bg-gray-900 p-4 rounded h-[100px] sm:h-[120px] overflow-hidden">
                  ...
                  {/* Dropdown Menu - Improved positioning for mobile */}
                  {activeMenu === resume.id && (
                    <div className="absolute right-0 top-12 w-40 bg-gray-800 rounded shadow-lg z-10 border border-gray-700">
                      <ul className="py-1">
                        <li>
                          <button
                            onClick={(e) => handleDuplicateResume(resume.id || '', e)}
                            className="w-full text-left px-4 py-3 text-sm text-gray-300 hover:bg-gray-700"
                          >
                            Duplicate
                          </button>
                        </li>
            ...
            <div className="bg-gray-800/40 rounded-lg overflow-hidden">
              <div className="p-6 flex flex-col items-center justify-center min-h-[150px] sm:min-h-[200px]">
        ...
        {/* Add Section Button - more tappable on mobile */}
        <div className="mt-6 sm:mt-8">
          <button 
            onClick={handleCreateNew} 
            className="flex items-center gap-2 px-4 py-3 border border-gray-700 text-gray-400 rounded-md hover:bg-gray-800/50 transition-colors"
          >
            <PlusCircle size={18} />
